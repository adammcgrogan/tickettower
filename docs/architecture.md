...
| `ticket_notes` | Private notes staff leave on a ticket (`/ticket note` or the Tickets page), with who wrote them; deleted with the transcript |
...
**Close requests.** `/ticket closerequest [reason] [close_after]` (support staff) sets `tickets.close_request_*` (`ticketbot/closerequest.go`, `store/close_requests.go`) and posts an embed with Close ticket and Keep open buttons only the opener can press. Close ticket closes the ticket as the opener, with the staff member's reason; Keep open withdraws the request and counts as the member's activity, as does any message from them (`RecordActivity`). A timed request that goes unanswered is closed by the auto-close loop on the requester's behalf (`CloseUnansweredRequest` is conditional, so an answer that lands first wins).
...
4. Every user has a level: managers are `owner`; a dashboard role grants `viewer` (read everything), `support` (also act on tickets: reply, close, reopen, move, hold, add or remove people, rename, block members) or `admin` (also change ticket types, buttons, saved replies and settings). The highest of a member's roles wins. `requireLevel` in `api/server.go` enforces the minimum per route group; only owners can change the dashboard roles (`validateSettings`). The guild response carries `level` (and `can_manage` for owners) so the frontend hides what the user can't do.
5. Transcripts (`/api/transcripts/{id}`) are visible to anyone with dashboard access, the ticket's opener, and that type's support roles. Anyone else gets a 404, so ticket IDs can't be probed. The ticket's private notes come with it for the team, but never for the member who opened it, even if they're on the team.
...
- `channels`, `roles`, `stats`, `setup-check`, `analytics?days=7|30|90|365|all&type={ticketTypeID}`, `tickets?status=open|closed&type={ticketTypeID}&q={search}&before={ticketID}&limit=` (newest first, at most 200 per page; `before` pages past a ticket; `q` matches the number, names and words in the ticket's messages, and a ticket found through its messages carries a `match` with a snippet), `tickets/{id}/close`, `tickets/{id}/reply`, `tickets/{id}/move`, `tickets/{id}/reopen`, `tickets/{id}/hold`, `tickets/{id}/resume`, `tickets/{id}/rename`, `tickets/{id}/notes` (all POST, support level), `tickets/{id}/members` (GET lists who has their own access, POST `{user_id}` adds someone) and `tickets/{id}/members/{userID}` (DELETE), `members?q=` (the server's members by name or pasted user ID), `settings` (GET/PATCH; PATCH is partial, so omitted fields are kept)
