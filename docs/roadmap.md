...
- Close requests: `/ticket closerequest [reason] [close_after]` asks the member to confirm their ticket is resolved, with Close ticket and Keep open buttons only they can press; unanswered timed requests (1 hour to 3 days, or only on confirmation) close on the staff member's behalf from the auto-close loop, and any message from the member withdraws the request
